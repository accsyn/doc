...
The recommended way is to install and use the accsyn desktop app for transferring files, it is way more stable and faster than attempting to use the browser. If it is small single files on the other hand, browser transfer will be sufficient.
...
- Download the installer and run it, accsyn will launch after installation has completed and then pick up the upload, prompting you which files to upload.
...
A file browser will appear where you can choose which folder to upload to. A new folder can be created by entering the name and clicking the plus (+) button:
...
4. Media will be added to the delivery cart on the right hand side, repeat the steps until you have collected all media. 
