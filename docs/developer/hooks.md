...
*Important Note: This feature is exposed to BYOS PRO workspaces only.*
...
- pre-publish-server;  Executed on the server when a remote user requests to publish one or more files, with publish file data provided and expecting feedback to be provided back to accsyn (see [Publish workflow](publish.md))
- publish-server;  Executed on the server when the published files from the remote user have been uploaded, with publish file data provided (see [Publish workflow](publish.md))

  

\* These client hooks only apply to clients owned by an elevated accsyn user (admin or employee), like for example a site server. They will not execute on standard user clients due to privacy and security concerns.
...
2. The configured script command line is loaded and the expression "${PATH\_JSON\_INPUT}" is replaced with the full absolute path to the temp JSON input file. This enables the script to load the metadata from arguments.
...
4. The script is executed by the same system user as accsyn (daemon) is running on the server.

## Hook data input format

### Job lifecycle hooks

Here follows example hook input data, as being passed on to the "post-done-server" hook after the file "moonshot.tif" was uploaded to a project share:
...
4. code & name; The job's human readable identifier.
5. source; accsyn raw source party ident, in the form "<entitytype>:<entityid>".
6. source\_hr; Source party in human readable form.
7. destination; accsyn raw destination party ident, in the form "<entitytype>:<entityid>".
8. destination\_hr; Destination party in human readable form.
...
17. created; The date the job was created.
18. queue; The ID of the parent queue the job is in.
19. queue\_hr; The name of the parent queue the job is in.
...
For detailed information about the publish hooks, head over to the [Publish documentation](publish.md).

## Configure hooks

To configure hooks globally, log on as an administrator to <https://accsyn.io/admin/settings>:
...
3. Click Enable hooks to have the hook subsystem enabled.
...
7. Choose the entry matching the operating system your server is running, enter the local full path to your script. Keep the token "${PATH\_JSON\_INPUT}" - it will be replaced with the path to the temp JSON input data at runtime. (Same goes for ${PATH\_JSON\_OUTPUT} with the pre-publish-server hook)

  

Click [ Save ] when done to apply your hooks configuration. Test the hook by submitting a job / performing a publish.
...
A tutorial going through how to set up an outsourcing pipeline with accsyn, using publish hooks.
