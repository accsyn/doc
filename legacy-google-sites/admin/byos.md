...
[Which additional features become available?](byos.md)

[Setting up](byos.md)

[Set up new BYOS workspace](byos.md)
...
Additionally, these [Workflow](../developer.md) features will be enabled:
...
You will be presented with an intro, read through carefully. Click Next when ready to install the server.
...
To set up BYOS, go to Billing (present in workspace menu on the left hand side) or open <https://accsyn.io/signup>.
...
You will be presented with an intro, read through carefully.
...
Click Next when the server is up and running.
...
As the last step, we need to configure the disk or folder on the server that accsyn should use. Click the Configure storage path button to proceed.

  

Enter the absolute path on the server, the path must exist and be writeable by the system user that runs the accsyn Daemon/server. 

  

Note: To change the user that the server runs as, please refer to [server documentation](byos/server.md).
...
1. The built-in "hq" site will now be set as the main site instead of "accsyn".
...
5. Workspace type will be set to byos and three (3) trial server licences will be generated with "storage", "site" and "compute" capabilities.

  

### Finalise

Click OK to reload the BYOS workspace,  you are now serving content from your own storage with no space limits and enabling advanced Workflows.
...
1. Make sure the file transfers perform as expected in regard to bandwidth utilisation.
...
You can now set up advanced Workflows with Hooks, Publishing and Render farm processing.
