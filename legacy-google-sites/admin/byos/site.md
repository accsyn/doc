...
[Related resouces](site.md)

## What is a site?

A site is a remote physical or cloud location that enables file synchronization and workflow collaboration across a geographically distributed organization.

  

Servers and clients owned by elevated (admin or employee role) users can be assigned to sites, defining were they are located. When signing up a workspace, a default cloud storage and server are initialised at the accsyn builtin site "hq" - providing basic cloud file delivery, file sharing and media vault storage.
...
- accsyn; This is the accsyn cloud storage site, and is primarily assigned new workspaces in the default accsyn setup before moving to BYOS. The accsyn site can be used in tandem with onprem storage.
...
- Show all sites; Display hidden built-in site such as the accsyn and roaming sites.
...
- Main - indicate if this the main site.
...
To create a new site, click NEW SITE button in upper right corner.
...
- Serve volume(s) at site, go to <https://accsyn.io/admin/volumes>, edit the volume that should be served by server and then go to "Servers" tab were you can set the server. Make sure storage is available at the site at the same physical disk locations as on configured at volume, you can override the paths if needed.
...
To edit a server, click on it in the list or choose Edit from the site's menu.

  

### Toolbar

- Disable site; Click this button to disable the site, note that all on jobs involving the site will be interrupted.
...
- Last checkin; When the server or client were last seen.
- Spawned; When it was created.
- ID; The internal (API) ID of server/client.

NOTE: Site and compute/render servers require additional BYOS licenses.
...
- Description; Set the optional description site should have.
...
Define metadata for this site, will be appended to upstream metadata and provided to jobs with Workflows - API calls, engine execution, hooks execution, and so on.

## Delete a site

To delete a site, open the server's menu (three dots icon) on the right hand side and choose Delete.
...
- The main site cannot be deleted, reach out to support to have a another site be set as the main site.
