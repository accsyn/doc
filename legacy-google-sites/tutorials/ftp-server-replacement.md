...
2. Download and logon to the accsyn [Desktop app](../admin/desktop-app.md), the app is used for managing file sharing.
...
More detailed information on BYOS workspace setup, can be found [here](../admin/byos.md).
...
Find more information about the accsyn Delivery feature [here](../delivery.md).
...
1. Logon as an administrator to <https://accsyn.io/admin/users>.
2. Click Invite user.
3. Choose employee role.
4. Select the volume or volumes you want to grant access to.
5. (Optional) Create a home share - give the user a private "home" folder were they can store files.
...
When they logon to the desktop app, they will get notifications about the two shared folders. In their file browser they will see "proj001" as a shared folder, with only the "source" and "FROM\_VENDOR" folders visible.

They will be able to only download from the source folder, and only see the "monique" folder beneath FROM\_VENDOR were they can upload files.
