...
The media exporter is designed to package video together with subtitles and other data, to provide a package that can be import by VOD streaming services.
...
- Licensing;  The date range platform can stream the title.
...
- Deliver (custom); Choose a location on storage were export is permanently stored, and then deliver from that location.
- Custom; Just store the export on storage, do nothing more.

  

When done,  click Export button to kick off export on the accsyn farm. You will be alerted on eventual missing entries.

  

Note: not all entries are required with the Allente exporter, for example the trailer might not be neccessary to include in some scenarios.
...
Check the farm - JOBS tab - at the bottom of the app GUI for progress on export, do not hesitate to reach out to the accsyn support if you encounter anu oddities withe export.
