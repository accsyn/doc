...
This guides shows how to process media files with accsyn - utilising the built-in transcoder engine to produce media variants and proxies of all forms and shapes.
...
- Remove eventual media that collides with media created by the processor.
...
The selected media are listed in the top section, below are a tabs with different available actions (click Cancel to abort processing):
...
Transcode master media to VOD media, optimised for SVOD/TVOD deliveris and further web stream (see Proxy below):
...
Create a HLS streamable HD proxy of VOD media, tailored for web streaming with the accsyn platform:
...
An image proxy is always created by default when videos are ingested into the vault, use this processor option to re-create it:

- Codec: JPG
- Standard quality: 320px maxwidthorheight, aspect ratiopreserved.
...
Re-probe metadata, check for physical existance on disk, check and re-sync with proxies,

## Troubleshooting

- I can't transcode VOD, it complains media is not a master? To be able to transcode VOD, you need to select video media that has the Cat(egory) tag set to "MAS". Edit the media and set this tag to enable VOD transcode.

- Transcoding takes forever and does not complete? The accsyn backend currently contains 10 high performance transcoding machines running 24/7 chrunching media. If you recon something has stuck, check the JOBS tab at the bottom area of the app. It shows your transcode job(s) and progress.

Having issues or questions you cannot resolve on your own, don't hesitate to reach out to our support team through Chat or email.
