...
Before you can start utilising accsyn title library capabilities, files and folders on your accsyn Cloud storage needs to be uploaded and ingested into the workspace media database.

Whenever you are in a title context and an uploaded file is recognised as media, accsyn ingest the file into the vault. Ingest means that the files is stored in the vault database as a media entity with metadata extracted and, if a picture or video file, proxy and thumbnail. 
...
1. In the app, go to the tile you want to ingest files to. Choose either the Files view or Media view, logging files in the Files view will suggest to preserve the folder structure (e.g. upload the the folder you are in) while logging in the Media view will have accsyn suggest the most appropriate folder depending on the type, content and category tags chosen. In the example we ingest to the media view:
...
- Based on content, category & type (default); This is the default when logging files in the media view with outside a folder context - have accsyn determine the most suitable folder structure for you based on the tags.
...
accsyn also tries to identify other tags based on the filename, you can adjust these later. These displayed as grey "pills" to the right. To remove files, click the trashcan button.
...
DCP:s are recognised if the folder contains a file named using the pattern '\*cpl\*.xml'. If the folder does not contain any file that matches this, it is not ingestable and will be treated as a folder.

## Ingesting uploaded media

Media can be uploaded in the general Storage view outside a title context, and be imported later inte the media vault.

1. Go to the title, or create the title on the folder were media file(s) are uploaded.
2. In the Files view, browse to the file.
3. Select the file(s) and click Ingest in the toolbar.
4. Proceed the same as you woud when logging media - choose destination folder / tag.
