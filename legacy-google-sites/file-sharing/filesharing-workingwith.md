...
2. Login with the desktop app as an elevated user - admin with full access or as an employee with access to at least one volume.
...
- Recent; The five(5) most recent visited shares.
- Volumes; Root folders/drives exposed to accsyn, the default cloud hostade volume is named "storage". In a BYOS setup, you are free to create and configure as many volumes as needed.
...
The base is a shared folder, ACLs are then applied to this folder, granting users access to the entire folder (/ path) or a subfolder for either downloading files or/and uploading files.

### Create a shared folder

1. In the Storage view, select the volume and browse to the folder you wish to share.
2. Click the blue Share button in toolbar and select Create shared folder, or click Create shared folder in the access panel to the right.
3. Give the share a name,:

![](../assets/file-sharing_filesharing-workingwith-3a4dda807107.png)

4. (Optional) Define the share code - the API identifier share should have when performing API operations.

5. (Optional) Bind a queue to the share - have all file transfers involving share go to a certain queue
...
5. Choose wich permissions user should have; Read means they will be able to list files and download, Write means they will be able to upload files, and alter files on storage - create directories, rename, move and delete files and folders.
...
2. Choose the user from list or invite a new user.
3. Click next.
4. (Optional) Give a description of share
...
To remove a share, right click it in the share list and choose Edit.
