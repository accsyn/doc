...
This is a guide for you as a end user, being granted access to download and/or upload to an accsyn shared folder or collection, using the accsyn Desktop app for the first time.
...
When someone gives you access to a folder within an accsyn workspace, you should have been receiving an email notification:

![](../assets/file-sharing_access-076839f5d814.png)

Example of a file sharing email notification

If it is the first time using accsyn the accsyn platform, you will also receive an invitation email telling you how to activate your personal accsyn account.

Note: If you can't find these emails, make sure to check your filtered spam inbox for emails. Search for the sender "[noreply@accsyn.com](mailto:noreply@accsyn.com)".
...
2. Login using the email account that were used when they shared the files with you - the email address notification above were sent to. The window will display and notify you that you have accessible items:
...
4. The download tool will appear, giving you the option to select which files and folders to download, and were to download them to locally:
...
Monitor the transfer by opening the My jobs view available at the bottom of the app interface, there you get an estimation of how long the transfer will take and get clues if anything goes wrong like disk gets full or there are permission/network errors.

## Upload files

The upload process is similar - launch the desktop app, login and click the Upload button to bring up the upload tool:

![](../assets/file-sharing_access-c37c9de72a51.png)

Example screenshot of the upload tool.

Choose the file(s) and/or folder(s) you wish to upload, under Destination choose the shared folder you wish to upload to. When done, click the Upload button to initiate upload.

## Advanced mode - transfer

If you are used to work with FTP clients, accsyn provides the Transfer view for this:
...
When you have made the selection, click the green arrow button in the middle to initate transfer.
...
If you run the installer as administrator on Windows, or have superuser privileges on Mac and Linux, these are the default installation location:
...
If you run the app as administrator on Windows, or with superuser privileges on Mac and Linux, these are the default data location:
...
Hint: make backup and migrate this location to new computers, in order to preserve app configuration such as login and file transfer client credentials.

  

### Log files location

accsyn store the log files in these locations:
...
To uninstall app as a standard user:
...
To install accsyn installed as an administrator on Windows, or with superuser privileges on Mac and Linux:
...
- Linux; Remove the accsyn application folder, typicall located in /usr/local.
...
I tried download a file but the job turns red and fails? 

- A first check is to see if you have enough free space on your hard drive were you chose to download the files/folders.
- Also check antivirus software and firewalls, so they are not blockin the executable or the default accsyn network ports - outgoing TCP traffic on ports 45190 and above.
- Make sure accsyn app has permission to write to the folder you chose for download, or can read the files you are about to upload.
...
Yes, accsyn facilitate standard AES -128 encryption as part of the SSL standard by default. File transfers are also accelerated using accsyn's proprietary network protocol, enabling fast and smooth deliveries for large file packages.

The desktop app freezes and/or crashes, what to do?

If you are having stability issues with the desktop app, please reach out to us by sending an email with your log files attached to [support@accsyn.com](mailto:support@accsyn.com). Find information in the Manage app section above on were the find the application logs on your local disk.

Having further issues or simply have question, please reach out to the accsyn support team through chat or by sending an email to [support@accsyn.com](mailto:support@accsyn.com).

Related articles:

[File sharing](../file-sharing.md)

- learn how to share files and folder with yoir employees and external users.

[Hosts](../admin/hosts.md)

- learn how to setup your computer to automatically receive deliveries, and expose locally mapped shares.
